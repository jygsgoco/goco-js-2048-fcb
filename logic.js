...
// If one of these value became true, the player wins the game
...
function setGame(){
	...
	/*board = [
        ...
        [16, 2, 256, 1024]
		 ...
		 // backend board
	// Goal, we will use the backend board to design and move the tiles of the front end and move the tiles of the frontend.
	...
	for (let c=0; c<columns ; c++){
		...
		for (let r=0 ; r<rows ; r++){
			...
			if(changedIndices.includes(r) && num !== 0) {
			    ...
			    tile.style.animation = "slide-from-bot 0.3s";
...
function checkWin(){
	for (let r=0; r < rows ; r++){
		for (let c=0 ; c < columns ; c++){
			...
			if (board[r][c] == 4096 && is4086exist == false){
